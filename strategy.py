...
def generate_events(df: pd.DataFrame) -> List[TradeEvent]:
    ...
    for _, row in df.iterrows():
        ...
        if position == "PE" and st_dir == -1:
            events.append(TradeEvent("EXIT", ts, "PE", close, "supertrend flipped bearish"))
            position = None

        elif position == "CE" and st_dir == 1:
            events.append(TradeEvent("EXIT", ts, "CE", close, "supertrend flipped bullish"))
            position = None

        # --- check entry (only if flat) ---
        if position is None:
            if close > r1 and st_dir == 1:
                events.append(TradeEvent("ENTRY", ts, "PE", close, "close>R1 & supertrend bullish"))
                position = "PE"
            elif close < s1 and st_dir == -1:
                events.append(TradeEvent("ENTRY", ts, "CE", close, "close<S1 & supertrend bearish"))
                position = "CE"
